...
export default function App() {
  ...
  // friendlier UX. We reuse the SDK's own `ChainSwitch` primitive so the
  // example stays honest about the SDK surface area it depends on.
  useEffect(() => {
    let cancelled = false;
  ...
  }, [selected, account, selectedInput.chainId, selectedChain]);
  ...
  async function handleExecute(quote: VtQuote) {
    ...
    try {
      ...
      // Identical to the privy-next-serverless example — the SDK has no idea
      // the WalletClient came from a hand-rolled EIP-6963 picker instead of
      // Privy's wrapper around the same EIP-1193 provider.
      await sdk.bridge({
        quote,
        walletClient,
        userAddress: account,
        ...
        onSuccess: async (result: BridgeResult) => {
          ...
          const dstAmount = formatUnits(BigInt(result.quote.dstAmount), USDM_DECIMALS);
