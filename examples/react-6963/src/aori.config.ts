...
// `VIEM_CHAINS_BY_ID` inside `App.tsx`, otherwise viem won't know how to
// build the WalletClient or pre-switch the wallet to it. The user's wallet
// itself only needs the chain present in its own networks list (most modern
// wallets will offer to add it via `wallet_addEthereumChain` on first use).
